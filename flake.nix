  ...
  outputs = {
    ...
    in {
      packages = let
        ...
        makeWrapper = name: minimal: script:
        ...
        arx' =
          pkgs.haskellPackages.arx.overrideAttrs
          (_: {
            version = "git";
            src = arxsrc;
            preConfigure = ''
        ...
        tarball = pkgs.runCommand "slurm-nix.tar.bz2" {} ''
        ...
        prune = pkgs.stdenv.mkDerivation {
          name = "prune";
          buildInputs = [(pkgs.haskellPackages.ghcWithPackages (p: with p; [sqlite-simple]))];
          unpackPhase = ''
            cp ${./prune.hs} ./prune.hs
          '';
          buildPhase = ''
            ghc -O -o prune ${./prune.hs}
          '';
          installPhase = ''
            install -Dm755 prune $out/bin/prune
          '';
        };
      in {
        inherit prune;
      ...
      defaultPackage = pkgs.runCommand "build-package" {} ''
